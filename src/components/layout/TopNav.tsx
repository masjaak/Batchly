...
export default function TopNav() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-20 items-center justify-between gap-4 border-b border-border bg-shell px-4 lg:px-6">
      <Link to="/app" className="flex items-center gap-2">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-ink text-sm font-semibold text-white">B</span>
        <span className="text-lg font-semibold text-ink">Batchly</span>
      </Link>
      ...
      <nav className="hidden items-center gap-1 lg:flex">
        {nav.map(({ to, label, end }) => (
          <NavLink key={to} to={to} end={end}
            className={({ isActive }) =>
              `rounded-xl px-4 py-2 text-sm font-medium transition-colors ${isActive ? 'bg-ink text-white' : 'text-secondary hover:text-ink'}`}>
            {label}
          </NavLink>
        ))}
      </nav>
      ...
      <div className="flex items-center gap-2">
        <div className="relative hidden items-center rounded-xl border border-border bg-surface lg:flex">
          <button onClick={() => toggle('search')} className="flex h-9 items-center px-3 text-sm text-secondary hover:text-ink">Cari</button>
          <button onClick={() => toggle('bell')} className="relative flex h-9 items-center px-3 text-sm text-secondary hover:text-ink">
            Notif
            {alerts.length > 0 && <span className="absolute right-1.5 top-2 h-1.5 w-1.5 rounded-full bg-ink" />}
          </button>
          <button onClick={() => toggle('info')} className="flex h-9 items-center px-3 text-sm text-secondary hover:text-ink">Info</button>
              ...
              <div className="absolute right-0 top-11 z-20 w-80 rounded-2xl border border-border bg-surface p-2 shadow-card">
                {panel === 'search' && (
                  <div>
                    <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari menu atau aksi…"
                      className="mb-1 h-10 w-full rounded-xl border border-border px-3 text-sm outline-none focus:border-ink" />
                      ...
                      {filtered.length === 0 ? <p className="px-3 py-3 text-sm text-secondary">Tidak ada hasil</p> :
                        filtered.map((i) => (
                          <button key={i.to} onClick={() => { navigate(i.to); setPanel(null); setQ('') }}
                            className="block w-full rounded-xl px-3 py-2 text-left text-sm text-ink hover:bg-surface-muted">{i.label}</button>
                        ))}
                    </div>
                  </div>
                )}
                ...
                {panel === 'info' && (
                  <div className="px-3 py-2">
                    ...
                    <Link to="/app/margin-guard" onClick={() => setPanel(null)} className="mt-2 inline-block text-xs font-medium text-ink underline">Pelajari Margin Guard</Link>
                  </div>
                )}
              </div>
            </>
        ...
        <Link to="/app/settings" className="flex items-center gap-2 rounded-xl border border-border bg-surface py-1 pl-1 pr-3">
